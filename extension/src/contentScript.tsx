...
// Initialize Gmail page filter
console.log("[KickYouAds] Initializing Gmail page filter...");
// The gmailPageFilter will automatically start listening for mode changes
...
function tryInsertButton(): boolean {
  ...
  if (targetContainer) {
    ...
    console.log(`[KickYouAds] 找到 ${possibleElements.length} 個可能相關的元素`);
    
    if (possibleElements.length > 0) {
      console.log('[KickYouAds] 前5個元素的類名:');
      Array.from(possibleElements).slice(0, 5).forEach((el, index) => {
...
console.log('[KickYouAds] Gmail page filter status:', gmailPageFilter.getStatus());

// 8. Listen for page navigation changes in Gmail (SPA navigation)
let currentUrl = window.location.href;
const urlCheckInterval = setInterval(() => {
  if (window.location.href !== currentUrl) {
    currentUrl = window.location.href;
    console.log('[KickYouAds] Page navigation detected, refreshing filter...');
    
    // Refresh the email filter after a delay to allow page content to load
    setTimeout(() => {
...
// 9. Add keyboard shortcut for testing
document.addEventListener('keydown', (event) => {
  // Ctrl+Shift+F to manually refresh filter
  if (event.ctrlKey && event.shiftKey && event.key === 'F') {
    event.preventDefault();
    console.log('[KickYouAds] Manual filter refresh triggered');
    gmailPageFilter.refresh();
  }
});
